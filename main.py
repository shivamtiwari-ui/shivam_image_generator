...
#     QWidget, QMessageBox, QApplication, QVBoxLayout, QLineEdit,
...
#         self.setGeometry((screen_geometry.width() - 800) // 2, (screen_geometry.height() - 600) // 2, 800, 600)
...
#         self.prompt_input.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
...
#         self.negative_prompt_input.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
...
#         self.cfg_scale_input.setValidator(QIntValidator(1, 30))
#         self.cfg_scale_input.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
...
#         self.steps_input.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
#         input_layout.addRow('Steps:', self.steps_input)

#         self.sampler_combo = QComboBox(self)
#         self.sampler_combo.addItems(samplers())
#         self.sampler_combo.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
#         input_layout.addRow('Sampler:', self.sampler_combo)

#         self.model_combo = QComboBox(self)
#         self.model_combo.addItems(models())
#         self.model_combo.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
#         input_layout.addRow('Model:', self.model_combo)

#         self.generate_button = QPushButton('Generate Image', self)
#         self.generate_button.setStyleSheet("background-color: #4CAF50; color: white; border: none; border-radius: 5px; padding: 10px; font-size: 16px;")
...
#         self.log_display.setStyleSheet("border: 1px solid #aaa; border-radius: 5px; padding: 5px;")
...
def generate_image(prompt, negative_prompt, model, cfg_scale, steps, sampler):
    api_url = "https://api.sitius.ir/"
    data = {
        "model": model,
        "prompt": prompt,
    ...
    with requests.Session() as session:
        response = session.post(f"{api_url}v1/generate/", json=data, headers=headers)
        job_id = response.json()
        
        while True:
            r = session.get(api_url + f"v1/image/{job_id}/")
            if r.status_code == 200:
                return r.json()  # Returns the image URL
            time.sleep(0.2)

# Streamlit App

st.title('OpenImagery - AI Image Generator')

# Sidebar inputs
st.sidebar.header('Input Parameters')
prompt = st.sidebar.text_area('Prompt', '')
negative_prompt = st.sidebar.text_area('Negative Prompt', 'Enter negative prompt here...')
cfg_scale = st.sidebar.slider('CFG Scale', 1, 30, 7)
steps = st.sidebar.slider('Steps', 1, 100, 30)
sampler = st.sidebar.selectbox('Sampler', samplers())
model = st.sidebar.selectbox('Model', models())

if st.sidebar.button('Generate Image'):
    if prompt:
        with st.spinner('Generating image...'):
            image_url = generate_image(prompt, negative_prompt, model, cfg_scale, steps, sampler)
